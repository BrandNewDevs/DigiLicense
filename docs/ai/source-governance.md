...
has an explicit source and section allowlist, so a relevant document cannot leak an unrelated section
into evidence. Critical numeric values are duplicated in fact packets so output validation can reject
unsupported dates, durations, and amounts.
